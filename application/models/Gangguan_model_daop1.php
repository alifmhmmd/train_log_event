<?php
class Gangguan_model_daop1 extends CI_Model{
    ...
    {
        $query = $this->db->select('id_jenis')->from('laporan_gangguan')->where('id_jenis', '1')->where('id_daop', '1')->get();
        if($query->num_rows()>0)
        {
    ...
    {
        $query = $this->db->select('id_jenis')->from('laporan_gangguan')->where('id_daop', '1')->where('id_jenis', '2')->get();
        if($query->num_rows()>0)
        {
            return $query->num_rows();
        }
    ...
    {
        $query = $this->db->select('id_jenis')->from('laporan_gangguan')->where('id_daop', '1')->where('id_jenis', '3')->get();
        if($query->num_rows()>0)
        {
            return $query->num_rows();
        }
    ...
    {
        $query = $this->db->select('id_jenis')->from('laporan_gangguan')->where('id_daop', '1')->where('id_jenis', '4')->get();
        if($query->num_rows()>0)
        {
            return $query->num_rows();
        }
    ...
    public function get_keyword($keyword)
    {
        $query = $this->db->select('*')
                        ->from('laporan_gangguan')
                        ->like('tempat_kejadian', $keyword)
                        ->or_like('tanggal', $keyword)
                        ->or_like('id_daop', $keyword)
                        ->or_like('id_jenis', $keyword);
