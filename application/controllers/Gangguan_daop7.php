...
class Gangguan_daop7 extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_daop7');
	}
	...
	public function index()
	{
        ...
        if($this->input->post('submit'))
        {
            ...
            $this->session->set_tempdata('keyword', $data['keyword'], 60); //set temp_data 60 second for searhing
        } else {
            $data['keyword'] = $this->session->tempdata('keyword');
        }
        //config pagination
        $config['base_url'] = base_url().'daop_7/gangguan';
        $this->db->like('tempat_kejadian', $data['keyword']);
        $this->db->from('laporan_gangguan');
        $this->db->where('id_daop', 7);
		...
		$data['laporan'] = $this->Gangguan_model_daop7->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_daop7->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_daop7->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_daop7->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_daop7->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_daop7->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_daop7->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_7/monitoring', $data);
    ...
    public function add_data()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar');
            $this->load->view('daop_7/input_gangguan',$data);
            $this->load->view('templates/footer');
        }else{
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Daop 7');
        ...
        $data = $this->Gangguan_model_daop7->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_daop7.xlsx';
