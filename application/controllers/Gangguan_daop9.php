...
class Gangguan_daop9 extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_daop9');
	}
	...
	public function index()
	{
		...
		$data['laporan'] = $this->Gangguan_model_daop9->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_daop9->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_daop9->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_daop9->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_daop9->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_daop9->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_daop9->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_9/monitoring', $data);
    ...
    public function add_data()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar');
            $this->load->view('daop_9/input_gangguan',$data);
            $this->load->view('templates/footer');
        }else{
            ...
            $this->Gangguan_model_daop9->save($data);
	...
	{
		$this->Gangguan_model_daop9->delete($id_laporan);
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Daop 9');
        ...
        $data = $this->Gangguan_model_daop9->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_daop9.xlsx';
