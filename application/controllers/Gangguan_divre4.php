...
class Gangguan_divre4 extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_divre4');
	}
	...
	public function index()
	{
		...
		$data['laporan'] = $this->Gangguan_model_divre4->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_divre4->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_divre4->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_divre4->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_divre4->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_divre4->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_divre4->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('divre_4/monitoring', $data);
    ...
    public function add_data()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar');
            $this->load->view('divre_4/input_gangguan',$data);
            $this->load->view('templates/footer');
        }else{
            ...
            $this->Gangguan_model_divre4->save($data);
            ...
            redirect(base_url("divre_4/gangguan"));
        }
    }

    public function delete_data($id_laporan)
	{
		$this->Gangguan_model_divre4->delete($id_laporan);
        $this->session->set_flashdata('message', '  <div class="alert alert-success solid alert-dismissible fade show">
                                                        <button type="button" class="close h-100" data-dismiss="alert" aria-label="Close"><span><i class="mdi mdi-close"></i></span>
                                                        </button>
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Divre 4');
        ...
        $data = $this->Gangguan_model_divre4->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_divre4.xlsx';
