...
class Gangguan extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_daop1');
	}
	...
	public function index()
	{
		...
		$data['laporan'] = $this->Gangguan_model_daop1->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_daop1->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_daop1->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_daop1->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_daop1->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_daop1->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_daop1->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_1/monitoring', $data);
    ...
    public function add_data()
    {
		...
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_1/input_gangguan',$data);
		$this->load->view('templates/footer');
        }else
        {
            ...
            $this->Gangguan_model_daop1->save($data);
	...
	{
		$this->Gangguan_model_daop1->delete($id_laporan);
        $this->session->set_flashdata('message', '  <div class="alert alert-success solid alert-dismissible fade show">
                                                        <button type="button" class="close h-100" data-dismiss="alert" aria-label="Close"><span><i class="mdi mdi-close"></i></span>
                                                        </button>
    ...
    public function edit_data($id_laporan)
    {
        ...
        $this->Gangguan_model_daop1->update($id_laporan, $data);
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Daop 1');
        ...
        $data = $this->Gangguan_model_daop1->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_daop1.xlsx';
