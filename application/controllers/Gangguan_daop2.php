...
class Gangguan_daop2 extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_daop2');
	}
	...
	public function index()
	{
		...
		$data['laporan'] = $this->Gangguan_model_daop2->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_daop2->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_daop2->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_daop2->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_daop2->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_daop2->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_daop2->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_2/monitoring', $data);
    ...
    public function add_data()
    {
		...
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_2/input_gangguan',$data);
		$this->load->view('templates/footer');
        }else
        {
            ...
            $this->Gangguan_model_daop2->save($data);
	...
	{
		$this->Gangguan_model_daop2->delete($id_laporan);
        $this->session->set_flashdata('message', '  <div class="alert alert-success solid alert-dismissible fade show">
                                                        <button type="button" class="close h-100" data-dismiss="alert" aria-label="Close"><span><i class="mdi mdi-close"></i></span>
                                                        </button>
    ...
    public function edit_data($id_laporan)
    {
        ...
        $this->Gangguan_model_daop2->update($id_laporan, $data);
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Daop 2');
        ...
        $data = $this->Gangguan_model_daop2->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_daop2.xlsx';
