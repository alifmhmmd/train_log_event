...
class Gangguan_daop4 extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Gangguan_model_daop4');
	}
	...
	public function index()
	{
        ...
        if($this->input->post('submit'))
        {
            ...
            $this->session->set_tempdata('keyword', $data['keyword'], 60); //set temp_data 2 second for searhing
        } else {
            $data['keyword'] = $this->session->tempdata('keyword');
        }
        //config pagination
        $config['base_url'] = base_url().'daop_4/gangguan';
        $this->db->like('tempat_kejadian', $data['keyword']);
        $this->db->from('laporan_gangguan');
        $this->db->where('id_daop', 4);
		...
		$data['laporan'] = $this->Gangguan_model_daop4->getAllData($config['per_page'],  $data['start'],  $data['keyword']);
		$data['daerah'] = $this->Gangguan_model_daop4->getDataDaerahOperasi();
        $data['jenis'] = $this->Gangguan_model_daop4->getDataJenisGangguan();
		$data['jumlah_sintelis'] = $this->Gangguan_model_daop4->getJumlahDataSintelis();
		$data['jumlah_jj'] = $this->Gangguan_model_daop4->getJumlahDataJJ();
		$data['jumlah_eksternal'] = $this->Gangguan_model_daop4->getJumlahDataEksternal();
		$data['jumlah_kamtib'] = $this->Gangguan_model_daop4->getJumlahDataKamtib();

        $data['user'] = $this->db->get_where('user', ['nipp' => $this->session->userdata('nipp')])->row_array();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_4/monitoring', $data);
    ...
    public function add_data()
    {
		...
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('daop_4/input_gangguan',$data);
		$this->load->view('templates/footer');
        }else
        {
            ...
            $this->Gangguan_model_daop4->save($data);
	...
	{
		$this->Gangguan_model_daop4->delete($id_laporan);
        $this->session->set_flashdata('message', '  <div class="alert alert-success solid alert-dismissible fade show">
                                                        <button type="button" class="close h-100" data-dismiss="alert" aria-label="Close"><span><i class="mdi mdi-close"></i></span>
                                                        </button>
    ...
    public function edit_data($id_laporan)
    {
        ...
        $this->Gangguan_model_daop4->update($id_laporan, $data);
        $this->session->set_flashdata('message', '  <div class="alert alert-success solid alert-dismissible fade show">
                                                        <button type="button" class="close h-100" data-dismiss="alert" aria-label="Close"><span><i class="mdi mdi-close"></i></span>
                                                        </button>
    ...
    public function export_data_excel()
    {
        ...
        $sheet->setCellValue('A1', 'Data Laporan Gangguan - Daop 4');
        ...
        $data = $this->Gangguan_model_daop4->getData();
        $x= 4;
        foreach ($data as $row) {
            $no=1;
        ...
        $filename = 'laporan_gangguan_daop4.xlsx';
